...
class MariColorToolkit:
    ...
    Modes:
      - ColorAdjust: brightness, contrast, saturation, gamma, exposure, hue_shift
      - HexImage   : generate solid color image from HEX
    Returns: (IMAGE)
    """
    ...
    def _apply_exposure(self, img, exposure):
        if exposure is None or abs(exposure) < 1e-6:
            return img
        factor = float(2.0 ** exposure)
        return _clamp01(img * factor)
    ...
    def run(self, mode, image=None,
        ...
        if mode == "ColorAdjust":
            if image is None:
                raise ValueError("ColorAdjust mode requires 'image' input.")
...
class MariImageResize:
    ...
    - size_preset: "Custom" or common resolutions
    """
...
class MariLoadCombo:
    ...
    - Load Checkpoint
    - Apply up to 6 LoRAs
    - Optional VAE override, CLIP skip
    - Output: MODEL, CLIP, VAE, family(STRING), info_json(STRING)
    - Supports manual overrides if dropdowns are empty
    """
    ...
    def run(self,
        ...
        for enabled, name, sm, sc in slots:
            if enabled and name and name != "(None)":
                ...
                applied_loras.append({"name": name, "strength_model": float(sm), "strength_clip": float(sc)})
